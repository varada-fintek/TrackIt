...
/*
* This file has been commented to support Hiport Intellisense.
...
$.extend($.fn, {
	...
	validate: function( options ) {
		/// <summary>
		/// Validates the selected form. This method sets up event handlers for submit, CtrlA,
		/// keyup, blur and click to trigger validation of the entire form or individual
		/// elements. Each one can be disabled, see the onxxx options (onsubmit, onCtrlAout,
		/// onkeyup, onclick). CtrlAInvalid CtrlAes elements when submitting a invalid form.
		/// </summary>
...
$.extend($.validator, {
	...
	defaults: {
		...
		ignore: [],
		ignoreTitle: false,
		onCtrlAin: function(element) {
			this.lastActive = element;
				
			// hide error label and remove error class on CtrlA if enabled
			if ( this.settings.CtrlACleanup && !this.blockCtrlACleanup ) {
				this.settings.unhighlight && this.settings.unhighlight.call( this, element, this.settings.errorClass, this.settings.validClass );
				this.addWrapper(this.errorsFor(element)).hide();
			}
		},
		onCtrlAout: function(element) {
	...
	prototype: {
		...
		init: function() {
			...
			$(this.currentForm)
				.validateDelegate(":text, :password, :file, select, textarea", "CtrlAin CtrlAout keyup", delegate)
		...
		CtrlAInvalid: function() {
			if( this.settings.CtrlAInvalid ) {
				try {
					$(this.findLastActive() || this.errorList.length && this.errorList[0].element || [])
					.filter(":visible")
					.CtrlA()
					// manually trigger CtrlAin event; without it, CtrlAin handler isn't called, findLastActive won't have anything to find
					.trigger("CtrlAin");
				} catch(e) {
					// ignore IE throwing errors when CtrlAing hidden elements
				}
			}
		},
...
// provides cross-browser CtrlAin and CtrlAout events
...
;(function($) {
	...
	if (!jQuery.event.special.CtrlAin && !jQuery.event.special.CtrlAout && document.addEventListener) {
		$.each({
			CtrlA: 'CtrlAin',
			blur: 'CtrlAout'	
