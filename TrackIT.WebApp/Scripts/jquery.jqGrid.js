			...
			if(!$t.p.multiselect) {	
				if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow !== pt.id) {
						...
						$(pt).addClass("ui-state-highlight").attr({"aria-selected":"true", "tabindex" : "0"});//.CtrlA();
						if(fid) {
...
$.jqm = {
hash:{},
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=(parseInt(h.w.css('z-index')));z=(z>0)?z:3000;var o=$('<div></div>').css({height:'100%',width:'100%',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});if(h.a)return F;h.t=t;h.a=true;h.w.css('z-index',z);
...
e=function(h){f(h);},
f=function(h){try{$(':input:visible',h.w)[0].CtrlA();}catch(_){}},
...
//Helper functions
	...
	viewModal : function (selector,o){
		...
		if ($.fn.jqm && o.jqm === true) {
			...
			try{$(':input:visible',selector)[0].CtrlA();}catch(_){}
		}
	},
...
$.jgrid.extend({
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					fl.find(".input-elm:CtrlA").change();
		...
		return this.each(function(){
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,status){
							...
							try{$(':input:visible',"#"+frmgr)[0].CtrlA();} catch (e){}
						}
			...
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				...
				$("#nNew","#"+$.jgrid.jqID(IDs.themodal)).click(function(){
					$(".confirm","#"+$.jgrid.jqID(IDs.themodal)).hide();
					$("#"+frmgr).data("disabled",false);
					setTimeout(function(){$(":input:visible","#"+frmgr)[0].CtrlA();},0);
					return false;
				});
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function CtrlAaref(){ //Sfari 3 issues
				if(rp_ge[$t.p.id].closeOnEscape===true || rp_ge[$t.p.id].navkeys[0]===true) {
					setTimeout(function(){$(".ui-jqdialog-titlebar-close","#"+$.jgrid.jqID(IDs.modalhead)).CtrlA();},0);
				}
			}
			function createData(rowid,obj,tb,maxcols){
					...
					.click(function(){
						if (!$(this).hasClass('ui-state-disabled')) {
							...
							if(dr){
								...
								$.jgrid.viewModal("#"+alertIDs.themodal,{gbox:"#gbox_"+$.jgrid.jqID($t.p.id),jqm:true});$("#jqg_alrt").CtrlA();
							}
						}
						return false;
					}).hover(onHoverIn, onHoverOut);
...
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		// End compatible
		return this.each(function(){
			var $t = this, nm, tmp, editable, cnt=0, CtrlA=null, svr={}, ind,cm, bfer;
			if (!$t.grid ) { return; }
			...
			if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
				...
				if(cnt > 0) {
					...
					$(ind).attr("editable","1");
					setTimeout(function(){$("td:eq("+CtrlA+") input",ind).CtrlA();},0);
					if(o.keys===true) {
				...
				$($t).jqGrid('navButtonAdd', elem,{
					...
					onClickButton : function () {
						...
						if(sr) {
							...
							$.jgrid.viewModal("#alertmod",{gbox:"#gbox_"+gID,jqm:true});$("#jqg_alrt").CtrlA();							
						}
					}
				});
			}
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if ($t.p.savedRow.length>0) {
				...
				window.setTimeout(function () { $("#"+$.jgrid.jqID($t.p.knv)).attr("tabindex","-1").CtrlA();},0);
			}
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$.jgrid.bindEv.call($t, elc, opt);
				window.setTimeout(function () { $(elc).CtrlA();},0);
	...
	restoreCell : function(iRow, iCol) {
		return this.each(function(){
			...
			window.setTimeout(function () { $("#"+$t.p.knv).attr("tabindex","-1").CtrlA();},0);
		});
	},
