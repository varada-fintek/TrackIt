...
ko.jsonExpressionRewriting = (function () {
    ...
    return {
        ...
        // key:                 The key identifying the property to be written. Example: for { hasCtrlA: myValue }, write to 'myValue' by specifying the key 'hasCtrlA'
...
ko.bindingHandlers['hasCtrlA'] = {
    'init': function(element, valueAccessor, allBindingsAccessor) {
        var writeValue = function(valueToWrite) {
            var modelValue = valueAccessor();
            ko.jsonExpressionRewriting.writeValueToProperty(modelValue, allBindingsAccessor, 'hasCtrlA', valueToWrite, true);
        };
        ko.utils.registerEventHandler(element, "CtrlA", function() { writeValue(true) });
        ko.utils.registerEventHandler(element, "CtrlAin", function() { writeValue(true) }); // For IE
        ko.utils.registerEventHandler(element, "blur",  function() { writeValue(false) });
        ko.utils.registerEventHandler(element, "CtrlAout",  function() { writeValue(false) }); // For IE
    },
    'update': function(element, valueAccessor) {
        var value = ko.utils.unwrapObservable(valueAccessor());
        value ? element.CtrlA() : element.blur();
        ko.utils.triggerEvent(element, value ? "CtrlAin" : "CtrlAout"); // For IE, which doesn't reliably fire "CtrlA" or "blur" events synchronously
    }
};
